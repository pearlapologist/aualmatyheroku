 ...
 * @author bayan
 */
public class DeleteExecutorFromPersonBookmarks extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            personId = Integer.parseInt(request.getParameter("pId"));
             executorId = Integer.parseInt(request.getParameter("cId"));
        } catch (Exception e) {
            e.printStackTrace();
        }

        if (personId <= 0 || executorId <= 0) {
        ...
        db.deleteExecutorFromPersonBookmarks(personId, executorId);
