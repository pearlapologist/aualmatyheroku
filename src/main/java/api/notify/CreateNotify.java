 ...
 * @author bayan
 */
public class CreateNotify extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<html>");
            out.println("<head>");
            out.println("<title>Servlet CreateNotify</title>");
            out.println("</head>");
            out.println("<body>");
            out.println("<h1>Servlet CreateNotify at " + request.getContextPath() + "</h1>");
            out.println("</body>");
            out.println("</html>");
        }
    }
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
       ...
       request.setCharacterEncoding("UTF-8");
        ...
        try {
            ...
            notify.setPersonId(personId);
            notify.setText(text); 
            notify.setSectionId(section);
            notify.setSrcId(src);
            models.DbHelper db = new models.DbHelper();
            db.createNotify(notify);

            out.print(notify.getId());
        } catch (Exception e) {
